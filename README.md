...
태아 상태 악화를 예측하는 소형 CNN(CTG-net 기반, 약 2,100 파라미터)을 학습하고 정답
라벨·신호 구간·입력 구성의 영향을 체계적으로 평가했다. 본 제안의 방법론이 이 연구의 두 축
...
| `model.py` | CTG-net (Ogasawara et al., 2021)의 PyTorch 재구현 + 논문의 1채널·메타데이터 변형 |
| `run_all.py` | 552건 전수 실행 — 두 파이프라인을 전 레코드에 적용 (단일 CPU 3.4초) |
| `log/ctu_features17.csv` | 552행 × (라벨 + 전처리 통계 + 특성 17개) |
| `log/ctu_model_inputs.npz` | 552건의 CTG-net 입력 텐서 (552 × 2 × 1,800) + 분석 대상 마스크 |
| `log/summary.md` | 논문 보고값과의 대조 — 라벨 분포 3종 정확 일치, 분석 대상 496건의 재구성 |
...
재구현 과정에서 논문이 밝히지 않은 사실 하나를 역산했다. 논문은 552건 중 **496건**을
분석에 썼다고만 적고 기준은 밝히지 않는다. 이 데이터셋의 원신호는 전부 60분 이상이라 30분
크롭 자체는 552건 모두 가능하므로 길이 기준일 수 없는데, **크롭 창의 잔여 결측이 1/3
이하**인 레코드만 남기면 정확히 496건이 된다(평활 전/후 어느 신호에서 재도 같다).
`log/summary.md`에 역산 과정과 함께 가설로 기록했다.
...
정의의 준거를 분명히 했다. Chiou et al.의 17개와 겹치는 인자(기저선·변이도·가속·감속과 그
유형·수축)는 그들의 Appendix A.2 계산 규칙을 그대로 따르며, 구현은
`../2-google-reproduction/features.py`를 직접 임포트해 재현 실험과 인자 추출이 같은 코드를
쓴다 — 정의가 갈라질 여지를 없앴다. ⑥의 결합 상관 계열은 A.2에 없는 본 제안의 추가
...
위 그림의 세그먼트(제대동맥혈 pH 7.14 레코드)에서 첫 감속은 수축 개시 20초 뒤에 시작해
수축 종료 전이므로 **후기**, 둘째는 그 조건을 벗어나고 3분 미만이라 **조기**로 분류되며, 둘
다 개시에서 최저점까지 30초 미만이라 **변이**로도 함께 계수된다 — 조기·후기·장기·중증이
...
3. **결합 상관 — 창별 피어슨 vs 전역 표준화 후 곱 평균.** 전역 표준화는 창마다 중심이
   어긋나 상관 크기를 0 쪽으로 누른다 (창별 −0.80이 0.00으로 나오는 세그먼트 확인).
