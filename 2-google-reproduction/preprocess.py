...
[재구현 선택] UC 결측의 식별: 논문은 "missing segments in both FHR and UC"를
보간한다고만 쓰고 UC 결측의 정의는 밝히지 않는다. 여기서는 FHR과 같은 기준
(값 0)을 쓴다 — TOCO 안정기의 실제 0과 구분할 수 없다는 한계가 있으나,
15초 초과 0-런은 어차피 0으로 유지되므로 영향은 짧은 0-런의 보간에 한정된다.
"""
...
def downsample_to_1hz(x, fs=FS):
    ...
    [재구현 선택] 다운샘플 방식(블록 평균 vs 데시메이션)은 논문에 없다. 블록
    평균은 결측 경계 블록에서 0 표지가 유효 표본과 섞여 값을 끌어내린다 —
    긴 결측의 내부는 정확히 0으로 유지된다.
    """
    n = len(x) // fs * fs
