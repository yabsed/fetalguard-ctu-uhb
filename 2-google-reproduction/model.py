...
Chiou et al. 변형:
  ...
  - 필터 수는 아키텍처 탐색 대상 (500개 무작위 구성 중 검증 AUROC 최고 선택)

학습 하이퍼파라미터는 medRxiv 프리프린트 Appendix B에 명시된 탐색 시 고정값을
따른다: BCE 손실, 학습률 3e-4, 배치 128, dropout 0.2. 커널·풀 크기와 필터 수는
어느 판에도 공개되지 않아 [재구현 선택] 값을 기본값으로 둔다 — 필터 4개는
원논문의 "4–8 filters" 범위 하한이자 파라미터 수(~2.1k)와 일치하는 조합이다.
"""
